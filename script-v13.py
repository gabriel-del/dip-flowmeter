...
while True:
  ...
  cX = int(M["m10"] / M["m00"])
  cY = int(M["m01"] / M["m00"])
  cv2.circle(circles_openning, (cX, cY), 5, (0,0,255), -1)
  # cv2.putText(circles_openning, "centroid", (cX - 25, cY - 25),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
  print(cX, cY)
  cv2.imshow("circles", circles_openning)
  redImg = np.zeros(circles_fatia.shape, image.dtype)
  redImg[:] = (255, 255, 0)
  redMask = cv2.bitwise_and(redImg, redImg, mask=circles_openning)
  cv2.addWeighted(redMask, 1, lines_fatia, 1, 0, lines_fatia)
